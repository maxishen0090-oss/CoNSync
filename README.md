# CoNSync 1.74 (Experimental): AirPlay-Mirror and AirPlay-Audio server for Linux, macOS, and Unix (also runs on Windows).

### **Now developed at the GitHub site <https://github.com/FDH2/CoNSync> (where ALL user issues should be posted, and latest versions can be found).**
...
-   **NEW in v 1.74 (Experimental) (June 2026)
     ...
     **This is a trial of a future release with new contributed code that provides
     ...
     CoNSync using `cmake -DUSE_DNS_SD=1`**    (By default, CoNSync on macOS continues to use
     Bonjour: to test the CoNSync internal mdns implementation on macOS, use `cmake -DUSE_MDNS=1`).
     ...
     *Comments about issues with this new internal mdns implementation, and
     ...
     are extremely welcome.*  (Issue [#529](https://github.com/FDH2/CoNSync/issues/529)
...
-   Support for **service discovery using a Bluetooth LE "beacon"** for both Linux/\*BSD and Windows (as an alternative to Bonjour/Rendezvous DNS-SD
    ...
    beacon is managed by a Python >= 3.6 script `CoNSync-beacon.py`. Loadable Python modules provide appropriate Bluetooth LE  support for Linux, Windows,
    ...
    host as a serial modem (this can be used on all the operating systems supported by CoNSync, including other BSD variants)._    The beacon runs independently
    of CoNSync.  While CoNSync is running, the beacon  regularly broadcasts a Bluetooth LE ("Low Energy") 46 byte
    legacy-type advertisement  informing nearby iOS/macOS devices of
    the local IPv4 network address of the CoNSync server, and which TCP port to contact CoNSync on.  Instructions
    are [given below](#bluetooth-le-beacon-setup).

-   option `-vrtp <rest-of-pipeline>`  bypasses rendering by CoNSync, and instead
...
-   (for Linux/*BSD Desktop Environments using D-Bus). New option `-scrsv <n>` provides screensaver inhibition (e.g., to
    ...
    activity): n = 0 (off) n=1 (on during video activity) n=2 (always on while CoNSync is running).
...
-   option -ca (with no filename given) will now render
    ...
    CoNSync. (-ca `<filename>` will continue to export cover art for
...
-   Improved Support for (YouTube) HLS (HTTP Live Streaming)
    ...
    **Please report any issues with this new feature of CoNSync**.
...
-   macOS computers (2011 or later, both Intel and "Apple Silicon" M1/M2
    ...
    CoNSync. Using AirPlay, CoNSync can emulate a second display for macOS
    clients.
...
[![Current Packaging
status](https://repology.org/badge/vertical-allrepos/CoNSync.svg)](https://repology.org/project/CoNSync/versions).

-   Install CoNSync on Debian-based Linux systems with
    "`sudo apt install CoNSync`"; on FreeBSD with
    "`sudo pkg install CoNSync`"; on OpenBSD with
    "`doas pkg_add CoNSync`". Also available on Arch-based systems
    through AUR. Since v. 1.66, CoNSync is now also packaged in RPM
    format by Fedora 38 ("`sudo dnf install CoNSync`").

-   For other RPM-based distributions which have not yet packaged
    CoNSync, a RPM "specfile" **CoNSync.spec** is now provided with recent
    [releases](https://github.com/FDH2/CoNSync/releases) (see their
    "Assets"), and can also be found in the CoNSync source top directory.
    See the section on using this specfile for [building an installable
    RPM package](#building-an-installable-rpm-package).

-   If your distribution does not supply CoNSync, or you want the latest version,
    it is very easy to build it yourself: see the very
    [detailed instructions for building CoNSync from source](#building-CoNSync-from-source).
    later in this document. 

## After installation:

-   (On Linux and \*BSD): if a firewall is active on the server hosting
    CoNSync, make sure the default network port (UDP 5353) for
    ...
    CoNSync, and use the "CoNSync -p `<n>`{=html}" option (see
    "`man CoNSync`" or "`CoNSync -h`").

-   Even if you install your distribution's pre-compiled CoNSync binary
    package, you may need to read the instructions below for [running
    CoNSync](#running-CoNSync) to see which of your distribution's
    **GStreamer plugin packages** you should also install.

-   For Audio-only mode (Apple Music, etc.) best quality is obtained
    with the option "CoNSync -async", but there is then a 2 second
    latency imposed by iOS.  Use option "CoNSync -ca" to display any "Cover Art" that
    accompanies the audio.

-   If you are using CoNSync just to mirror the client's screen (without
    showing videos that need audio synchronized with video), it is best to
    use the option "CoNSync -vsync no".

-   Add any CoNSync options you want to use as defaults to a startup file
    `~/.CoNSyncrc` (see "`man CoNSync`" or "`CoNSync -h`" for format and
    other possible locations; the location can also be set with "CoNSync -rc _location_"). 
...
-   For Linux/*BSD systems using D-Bus, the option `-scrsv 1` inhibits the screensaver while
    there is video activity on CoNSync (`-scrsv 2` inhibits it whenever CoNSync is running).

-   For Linux systems using systemd, there is a **systemd** service file **CoNSync.service**
    found in the CoNSync top directory of the distribution, and also installed
    in `<DOCDIR>/CoNSync/systemd/` (where DOCDIR is usually ``/usr/local/share/doc``), that allows users to start
    their own instance of CoNSync as a rootless daemon: it should either be added to the
    directory /etc/systemd/user, or the  user can just create their own
    systemd directory `~/.config/systemd/user/` and then copy CoNSync.service into it.  To save
    CoNSync terminal output to a file ~/CoNSync.log, uncomment the StandardOutput entry in
    CoNSync.service. Then

    `systemctl --user [start/stop/enable/disable/status] CoNSync`
    ...
    systemd user services. If more than one user might simultaneously run CoNSync this way, they should
    specify distinct -p and -m options (ports and deviceID) in their startup files.
    **Note: it is NOT recommended to run CoNSync as a root service.**

-   On Raspberry Pi:  models using hardware h264 video decoding by the
    Broadcom GPU (models 4B and earlier) may require the CoNSync option -bt709.
    If you use Ubuntu 22.10 or earlier, GStreamer must
    be [patched](https://github.com/FDH2/CoNSync/wiki/Gstreamer-Video4Linux2-plugin-patches)
...
-   If CoNSync is used in a public space, there are security options for requiring an AppleTV-style
...
pulled from the new main [CoNSync site](https://github.com/FDH2/CoNSync)).

CoNSync is tested on a number of systems, including (among others) Debian
...
now also Microsoft Windows). CoNSync supports Apple's AirPlay2 protocol
...
The CoNSync server and its client must be on the same local area network,
...
Connections to the CoNSync server by iOS/MacOS clients can be initiated
...
mirroring. In **Audio** mode, metadata is displayed in the CoNSync
terminal; if CoNSync option `-ca <name>` is used, the accompanying cover
...
-   **Note that Apple video-DRM (as found in "Apple TV app" content on
    the client) cannot be decrypted by CoNSync, and the Apple TV app
    cannot be watched using CoNSync's AirPlay Mirror mode (only the
    unprotected audio will be streamed, in AAC format).**

-   **With the new "-hls" option, CoNSync now also supports non-Mirror
...
CoNSync uses [GStreamer](https://gstreamer.freedesktop.org) "plugins" for
...
-   **NVIDIA with proprietary drivers**
    ...
    be [built by the
    user](https://github.com/FDH2/CoNSync/wiki/NVIDIA-nvdec-and-nvenc-plugins).
...
-   **Video4Linux2 support for h264 hardware decoding on Raspberry Pi
    ...
    CoNSync using software video decoding, but hardware-accelerated
    h264/h265 decoding by firmware in the Pi's Broadcom 2835 GPU is
    prefered. CoNSync accesses this using the GStreamer-1.22 Video4Linux2
    ...
    [CoNSync
    Wiki](https://github.com/FDH2/CoNSync/wiki/Gstreamer-Video4Linux2-plugin-patches))*.
...
-   **Support for h265 (HEVC) hardware decoding on Raspberry Pi (Pi 4
    ...
    in the mainline kernel; without this support, CoNSync support for
    HEVC hardware decoding on Raspberry Pi will not work.

### Note to packagers:

CoNSync's GPLv3 license does not have an added "GPL exception" explicitly
...
# Building CoNSync from source

Either download and unzip
[CoNSync-master.zip](https://github.com/FDH2/CoNSync/archive/refs/heads/master.zip),
or (if git is installed): "git clone https://github.com/FDH2/CoNSync".
You can also download a recent or earlier version listed in
[Releases](https://github.com/FDH2/CoNSync/releases).

-   A recent CoNSync can also be found on the original [antimof
    site](https://github.com/antimof/CoNSync); that original project is
    inactive, but is usually kept current or almost-current with the
    [active CoNSync github site](https://github.com/FDH2/CoNSync) (thank
    you antimof!).

## Building CoNSync on Linux (or \*BSD):
...
gstreamer installation that CoNSync should use (if this is *e.g.*
...
-   Most users will use the GStreamer supplied by their distribution,
    ...
    instructions](https://github.com/FDH2/CoNSync/wiki/Building-latest-GStreamer-from-source-on-distributions-with-older-GStreamer-(e.g.-Raspberry-Pi-OS-).)
    . **Do this *before* building CoNSync**.

In a terminal window, change directories to the source directory of the
downloaded source code ("CoNSync-\*", "\*" = "master" or the release tag
for zipfile downloads, "CoNSync" for "git clone" downloads), then follow
the instructions below:

**Note:** By default CoNSync will be built with optimization for the
...
fullscreen mode with a keypress (F11 or Alt_L+Enter) CoNSync needs to be
built with a dependence on X11. Starting with CoNSync-1.59, this will be
...
-   If X11 development libraries are present, but you wish to build
    CoNSync *without* any X11 dependence, use the cmake option
    `-DNO_X11_DEPS=ON`.
...
This installs the executable file "`CoNSync`" to `/usr/local/bin`, (and
installs a manpage to somewhere standard like
`/usr/local/share/man/man1` and README files to somewhere like
`/usr/local/share/doc/CoNSync`). (If "man CoNSync" fails, check if
\$MANPATH is set: if so, the path to the manpage (usually
/usr/local/share/man/) needs to be added to \$MANPATH .) The CoNSync
...
\*\*For those with RPM-based distributions, a RPM spec file CoNSync.spec
...
"`rpmdev-setuptree`". Then download and copy CoNSync.spec into `~/rpmbuild/SPECS`. In
that directory, run "`pmdev-spectool -g -R  CoNSync.spec`" to download the corresponding
source file `CoNSync-*.tar.gz` into `~/rpmbuild/SOURCES`
("rpmdev-spectool" may also be just called "spectool"); then run
"`rpmbuild -ba CoNSync.spec`" (you will need to install any required
dependencies this reports). This should create the CoNSync RPM package in
a subdirectory of `~/rpmbuild/RPMS`. (**CoNSync.spec** is tested on
...
### Starting and running CoNSync


* A method to enable Service Discovery (so AirPlay clients on the local network can find
the  AirPlay service provided by CoNSync) must be set up.  The standard method used by CoNSync
is to  announce the service on the **mDNS network port UDP 5353, which must be opened for mdns queries if
a firewall is active on the host computer **.     Previously, CoNSync needed an external `DNS_SD` service
(Apple's Bonjour service, usually implemented on Linux/\*BSD by Avahi or mDNSResponder, but
CoNSync now also supplies  its own built-in 'minimal' version of mDNSResponder.   The choice between
...
Since CoNSync-1.64, CoNSync can be started with options read from a
configuration file, which will be the first found of (1) a file with a
path given by environment variable `$CONSYNCRC`, (2) `~/.CoNSyncrc` in the
user's home directory ("\~"), (3) `~/.config/CoNSyncrc`. The format is
...
**Run CoNSync in a terminal window**. On some systems, you can specify
...
If
CoNSync is seen, but the client fails to connect when it is selected,
there may be a firewall on the server that prevents CoNSync from
...
-   Unlike an Apple TV, the CoNSync server does not by default require
    ...
    need to repeat this). Since v1.67, CoNSync offers such
    ...
    devices) are required to use pin-authentication: CoNSync will provide
    ...
    to access CoNSync, when prompted (if _pwd_ is not set, a displayed random
    pin code must be entered at **each** new connection.)

-   By default, CoNSync is locked to its current client until that client
    drops the connection; since CoNSync-1.58, the option `-nohold`
    modifies this behavior so that when a new client requests a
    connection, it removes the current client and takes over. CoNSync
...
-   In Mirror mode, GStreamer has a choice of **two** methods to play
    video with its accompanying audio: prior to CoNSync-1.64, the video
    ...
    with CoNSync-1.64, the other method (GStreamer's "*sync=true*" mode),
    which uses timestamps in the audio and video streams sent by the
    client, is the new default**. On low-decoding-power CoNSync hosts
...
more powerful systems, and is still available with the CoNSync option
"`-vsync no`"; this method is adapted to "live streaming", and may be
better when using CoNSync as a second monitor for a Mac computer, for
...
-   In Audio-only mode the GStreamer "sync=false" mode (not using
    ...
    listening to superior sound on the CoNSync server). This delays the
...
-   You can now  display (inside CoNSync)  the accompanying "Cover Art" from
    sources like Apple Music in Audio-Only (ALAC) mode with the option
    `CoNSync -ca`. _The older method of exporting cover art to an external
    viewer remains available: run
    "`CoNSync -ca <name> &`" in the background, then run a image viewer
    with an autoreload feature: an example is "feh": run
    "`feh -R 1 <name>`" in the foreground; terminate feh and then CoNSync
...
to use. You can overide this with the CoNSync option `-vs <videosink>`.
...
decoding (e.g., VAAPI). Try "`CoNSync -avdec`" to force software video
...
-   CoNSync uses the Video4Linux2 (v4l2) plugin from GStreamer-1.22 and
...
-   On older distributions (GStreamer \< 1.22), the v4l2 plugin needs a
    patch: see the [CoNSync
    Wiki](https://github.com/FDH2/CoNSync/wiki/Gstreamer-Video4Linux2-plugin-patches).
    Legacy Raspberry Pi OS (Bullseye) has a partially-patched
    GStreamer-1.18.4 which needs the CoNSync option -bt709 (and don't use
    -v4l2); it is still better to apply the full patch from the CoNSync
    Wiki in this case.
...
-   **H265 (4K)** video is potentially supported by hardware decoding on
    ...
    present CoNSync cannot use this. The best solution would be for the
...
The basic CoNSync options for R Pi are `CoNSync [-vs <videosink>]`. The
...
* Tip: to start CoNSync on a remote host (such as a Raspberry Pi) using
    ssh:

```
ssh user@remote_host
       export DISPLAY=:0
       nohup CoNSync [options] > FILE &
```

Sound and video will play on the remote host; "nohup" will keep CoNSync
running if the ssh session is closed. Terminal output is saved to FILE
(which can be /dev/null to discard it)

## Building CoNSync on macOS: **(Intel X86_64 and "Apple Silicon" M1/M2 Macs)**
...
CoNSync can run on older macOS
...
uninstalled after building CoNSync, if you wish.
...
-   Since GStreamer v1.22, the "Official" (gstreamer.freedesktop.org)
    ...
    main program (CoNSync). This should have been applied during the
    CoNSync compilation process, and the initial CoNSync terminal message
    should confirm it is being used. (CoNSync can also be built using
...
complete GStreamer, but seems to have everything needed for CoNSync).
**New: the CoNSync build script will now also detect Homebrew
...
macOS, compile CoNSync using a special cmake option `-DUSE_X11=ON`, and
run it from an XQuartz terminal with -vs ximagesink; older non-retina
macs require a lower resolution when using X11: `CoNSync -s 800x600`.)*

After installing GStreamer, build and install CoNSync: open a terminal
and change into the CoNSync source directory ("CoNSync-master" for zipfile
downloads, "CoNSync" for "git clone" downloads) and build/install with
"cmake . ; make ; sudo make install" (same as for Linux).


-   **NEW**: CoNSync now optionally supplies its own minimal implementation of
    ...
    use the CoNSync implementation (which is tested and works on macOS), compile
    with `cmake -DUSE_MDNS=1`.
    

-   Running CoNSync while checking for GStreamer warnings (do this with
    "export GST_DEBUG=2" before runnng CoNSync) reveals that with the
    default (since CoNSync 1.64) use of timestamps for video
    ...
    GStreamer warnings. **Recommendation: use the CoNSync "no
    timestamp" option "`-vsync no`"** (you can add a line "vsync no" in
    the CoNSyncrc configuration file).
...
-   *On older releases of GStreamer, there were issues involving windows not
    closing properly, that required the CoNSync  option "-nc": these issues
...
## Building CoNSync on Microsoft Windows, using MSYS2 with the MinGW-64 compiler.

-   tested on Windows 10 and 11, 64-bit.

* **NEW: CoNSync now supplies its own self-contained mdns replacement for Bonjour, making step 1 unnecessary unless
...
3.  [MSYS2 packages](https://packages.msys2.org/package/) are installed
    ...
    * _NEW: MSYS2 now recommends using the newer UCRT64 terminal environment (which uses the newer Microsoft
    ...
    We will simply build CoNSync from the command line in the MSYS2
    environment (using "`ninja`" in place of "`make`" for the build
    system).

4.  Download the latest CoNSync from github **(to use `git`, install it
    with `pacman -S git`, then
    "`git clone https://github.com/FDH2/CoNSync`")**, then install CoNSync
...
5.  cd to the CoNSync source directory, then "`mkdir build`" and
    ...
    location. Then build CoNSync with

    `cmake ..`

    `ninja`

6.  Assuming no error in either of these, you will have built the CoNSync
    executable **CoNSync.exe** in the current ("build") directory. The
    ...
    CoNSync.exe executable in `C:/msys64/ucrt64/bin` (plus manpage and
    documentation in `C:/msys64/ucrt64/share/...`) with

    `cmake --install . --prefix $HOME/../../ucrt64`
    You can later uninstall CoNSync by returning to the build directory and running

    `ninja uninstall`

    (This assumes that certain files in the build directory were not deleted since building CoNSync). 

    To be able to view the manpage, you need to install the manpage
    viewer with "`pacman -S man`".

To run **CoNSync.exe** you need to install some gstreamer plugin packages
...
You also will need to grant permission to the CoNSync executable
CoNSync.exe to access data through the Windows firewall. You may
automatically be offered the choice to do this when you first run
CoNSync, or you may need to do it using **Windows Settings-\>Update and
Security-\>Windows Security-\>Firewall & network protection -\> allow an
app through firewall**. If your virus protection flags CoNSync.exe as
"suspicious" (but without a true malware signature) you may need to give
it an exception.

Now test by running "`CoNSync`" (in a MSYS2 UCRT64 terminal window.  If you need
...
such as
    ...
    CoNSync -as 'wasapisink device=\"<guid>\"' 
...
-   On Windows, with Direct3D 11.0 or greater, various options can be set
    ...
    **It is recommended that Windows users add a line `-vs d3d12videosink` in their CoNSync
...
The executable CoNSync.exe can also be run without the MSYS2 environment,
in the Windows Terminal, with `C:\msys64\ucrt64\bin\CoNSync`.
...
-   These can also be written (one option per line, without the initial
    "`-`" character) in the CoNSync startup file (either given by
    environment variable `$CONSYNCRC`, or `~/.CoNSyncrc` or
    `~/.config/CoNSyncrc`); lines begining with "`#`" are treated as
...
overrides `$CONSYNCRC`, ``~/.CoNSyncrc``, etc.

**-n server_name** (Default: CoNSync); server_name@\_hostname\_ will be
the name that appears offering AirPlay services to your iPad, iPhone
etc, where *hostname* is the name of the server running CoNSync. This
...
videos can be streamed directly from YouTube servers to CoNSync (without
...
should list CoNSync when it is inhibiting
the screensaver._
...
CoNSync startup file if you wish the CoNSync server to use the pin
authentication protocol).*

**-reg \[*filename*\]**: (since v1.68). If "-pin" is used, this option
maintains a register of pin-authenticated "trusted clients" in
\$HOME/.CoNSync.register (or optionally, in *filename*). Without this
option, returning clients that skip pin-authentication are trusted and
not checked. This option may be useful if CoNSync is used in a more
...
six characters) is set when CoNSync
starts (usually set in the startup file, where it is stored as
cleartext), all users must know this password to connect to CoNSync
...
validate the CoNSync password, and no user credentials are exposed.
...
the CoNSync deviceID has not changed (this initial authentication will fail
if the CoNSync password has changed).
If *pwd* is **not** specified with the -pw option when CoNSync starts, a
new random 4-digit pin code is generated and displayed on the CoNSync terminal
...
CoNSync-1.64. Standard desktop systems seem to work well without use of
timestamps: this mode is appropriate for "live streaming" such as using
CoNSync as a second monitor for a mac computer, or monitoring a webcam;
...
off a `-async` option set in a "CoNSyncrc" configuration file.
...
**-p** allows you to select the network ports used by CoNSync (these need
...
"`CoNSync -ca [path/to/]filename &`" in the background, then run the the
...
CoNSync was put into the background). To quit, use `ctrl-C fg ctrl-C` to
terminate the image viewer, bring `CoNSync` into the foreground, and
terminate it too.
...
and the file is deleted when CoNSync terminates.
...
**-nc** maintains previous CoNSync \< 1.45 behavior that does **not
...
ownership of CoNSync until it disconnects.
...
displayed by CoNSync when the client attempts to connect, and appears to
...
entry in the CoNSyncrc startup file.
...
be an entry in the CoNSyncrc startup file.
...
CoNSync at the same time. *This setting is only an advisory to the client
...
**-m \[mac\]** changes the MAC address (Device ID) used by CoNSync
...
network ports are needed for each running CoNSync if you attempt to run
more than one instance of CoNSync on the same computer.) If \[mac\] (in
form xx:xx:xx:xx:xx:xx, 6 hex octets) is not given, a random MAC address
is generated. If CoNSync fails to find the true MAC address of a network
...
different each time CoNSync is started).
...
`$HOME/.CoNSync.pem`, if that file does not exist, or read from it, if it
...
The port, PID and process name of the CoNSync process is recorded by default
in `~/.CoNSync.ble` : (this file is created
when CoNSync starts and deleted when it stops.)
Optionally the  file
...
CoNSync. To see GStreamer information messages, set GST_DEBUG=4; for
DEBUG messages, GST_DEBUG=5; increase this to see even more of the
GStreamer inner workings.

# Bluetooth LE beacon setup

The python>=3.6 script for running a Bluetooth LE Service Discovery beacon is CoNSync-beacon.py.
Besides the standard python libraries, it requires python3-psutils.

It provides four possible Bluetooth LE implementations (loaded as modules `CoNSync_beacon_module_*.py`), which may need additional libraries:
...
* **HCI** For Linux and FreeBSD: no extra libraries, but requires root access to the HCI (Host Controller Interface) for Bluetooth devices, using utilities
    ...
    and you can find it on the [CoNSync Wiki](https://github.com/FDH2/CoNSync/wiki/hccontrol-patch-for-FreeBSD-15.0). _Note that macOS
...
CoNSync must  be  run with option "`CoNSync -ble [path_to_file]`" (so it writes data for the Bluetooth beacon to the default BLE 
data file `~/.CoNSync.ble`, or optionally to the file specifed by its path), then run ``CoNSync-beacon.py [HCI, BleuIO] [--path path_to_file]`` in a separate terminal.
...
Bluetooth LE Service-Discovery advertising when it detects that CoNSync is running by checking if the BLE data file exists, and stop when it no longer detects
a running CoNSync (it will restart advertising if CoNSync later reappears).   The script will remain active until stopped with Ctrl+C in its
...
a configuration file `~/.CoNSync.beacon`, if it exists. Configuration file entries are like the command line forms, one per line (e.g.,
`--ipv4 192.168.1.100`).  Lines commented out with an initial ``#`` are ignored.  Command line options override the configuration file
options.   Get help with `man CoNSync-beacon` or ``CoNSync-beacon.py --help``.
...
* `--ipv4  <ipv4 address>`.  This option can be used to specify the ipv4 address at which the CoNSync server should be contacted by the client. If
...
* `--path <BLE data file>`.  This overrides the default choice of BLE data file (``~/.CoNSync.ble``) that is monitored by the beacon script.   This also requires
that CoNSync is run with option "`CoNSync -ble <BLE data file>`".
...
* `--index x` (default x = 0, x >= 0).  This can be used by the DBus to distinguish between  multiple simultaneous instances of CoNSync-beacon.py that
are running to support multiple instances of CoNSync.  Each instance must have its own BLE Data
file (just as each instance of CoNSync must also have its own MAC address and ports).  _Note:  running multiple beacons simultaneously
...
that sent for  AirPlay Service Discovery), and  the only current support of CoNSync-beacon.py on macOS uses a BleuIO USB serial device.
...
For more information on Bluetooth LE support, including HCI commands, see the [wiki page](https://github.com/FDH2/CoNSync/wiki/Bluetooth_LE_beacon).
...
# Troubleshooting
Note: `CoNSync` is run from a terminal command line, and informational
messages are written to the terminal.

### 0. Problems in compiling CoNSync.
...
required for CoNSync to work. On Linux, it will be usually provided by
...
service on the system running CoNSync (your distribution will document
...
-   **CoNSync starts, but either stalls or stops after "Initialized
    server socket(s)" appears (*without the server name showing on the
    client*)**.

If CoNSync stops with the "No DNS-SD Server found" message, this means
that your network **does not have a running Bonjour/zeroconf DNS-SD
server.** Before v1.60, CoNSync used to stall silently if DNS-SD service
...
If CoNSync stalls *without an error message* and *without the server name
showing on the client*, **this is a network problem** (if your CoNSync
...
-   Some users using dual-band (2.4GHz/5GHz) routers have reported that
    clients using the 5GHz band (sometimes) "fail to see CoNSync" (i.e.,
...
-   **Avahi works at first, but new clients do not see CoNSync, or
...
not listening when CoNSync started.

To check this, after starting CoNSync, use the utility
`avahi-browse -a -t` **in a different terminal window** on the server to
verify that the CoNSync AirTunes and AirPlay services are correctly
registered (only the AirTunes service is used in the "Legacy" AirPlay
Mirror mode used by CoNSync, but the AirPlay service is used for the
initial contact).

The results returned by avahi-browse should show entries for CoNSync like

    +   eno1 IPv6 CoNSync                                        AirPlay Remote Video local
    +   eno1 IPv4 CoNSync                                        AirPlay Remote Video local
    +     lo IPv4 CoNSync                                        AirPlay Remote Video local
    +   eno1 IPv6 863EA27598FE@CoNSync                           AirTunes Remote Audio local
    +   eno1 IPv4 863EA27598FE@CoNSync                           AirTunes Remote Audio local
    +     lo IPv4 863EA27598FE@CoNSync                           AirTunes Remote Audio local

If only the loopback ("lo") entries are shown, a firewall on the CoNSync
...
If the CoNSync services are listed by avahi-browse as above, but are not
seen by the client, the problem is likely to be a problem with the local
network.

### 2. CoNSync starts, but stalls after "Initialized server socket(s)" appears, *with the server name showing on the client* (but the client fails to connect when the CoNSync server is selected).

This shows that a *DNS-SD* service is working, clients hear CoNSync is
available, but the CoNSync server is not receiving the response from the
...
the range 1024-65535, opened for both tcp and udp, and use "CoNSync -p n"
(or open UDP 7011,6001,6000 TCP 7100,7000,7001 and use "CoNSync -p").
...
finished. For such problems, use "CoNSync -d" (debug log option) to see
...
from a successful start of CoNSync in the [CoNSync
Wiki](https://github.com/FDH2/CoNSync/wiki).

**If CoNSync reports that mirroring started, but you get no video or
...
before running CoNSync, or set this in the default environment.
...
CoNSync](#starting-and-running-CoNSync) for choices of `<audiosink>` or
`<videosink>`.)
...
(this is a GStreamer issue). You may need to terminate CoNSync with
...
If CoNSync issues no error messages, but GStreamer video seems not to be working,
...
Then test the various videosinks (see the -vs entry in `man CoNSync` for a list) to see
...
If CoNSync fails to start, with a message that a required GStreamer
...
-   starting with release CoNSync-1.65.3, CoNSync will continue to
...
will run CoNSync, so that you see warning and error messages; see
[GStreamer debugging
...
alternative to X11 reported that after reinstalling Lubuntu 18.4, CoNSync
...
`CoNSync -d`, has the default form
...
replaced by using CoNSync options `-vp`, `-vd`, `-vc`, and `-vs`, if
...
stream may continue to arrive). At 3-second intervals, CoNSync checks
...
(currently 5) of consecutive ntp timeouts occur, CoNSync assumes that the
...
causes CoNSync to reset the connection.
...
The protocol was modifed in CoNSync-1.65 after it was discovered that the
...
Legacy Pairing" (bit 27) in the "features" code CoNSync advertises on
...
simultaneous handling of multiple clients (CoNSync only supports one
client at a time). **This change is now well-tested, but in case it
causes any protocol failures, CoNSync can be reverted to the previous
...
CoNSync.** ("Pairing" is re-enabled when the new Apple-style one-time
"pin" authentication is activated by running CoNSync with the "-pin"
option introduced in CoNSync 1.67.)
...
`OLD_PROTOCOL_CLIENT_USER_AGENT_LIST` in `CoNSync/lib/global.h`. CoNSync
...
global.h and rebuild CoNSync.

Note that for DNS-SD Service Discovery, CoNSync declares itself to be an
AppleTV3,2 (a 32 bit device) with a sourceVersion 220.68; this can also
be changed in global.h. CoNSync also works if it declares itself as an
AppleTV6,2 with sourceVersion 380.20.1 (an AppleTV 4K 1st gen,
introduced 2017, running tvOS 12.2.1), so it does not seem to matter
what version CoNSync claims to be.

# Changelog
1.74  2026-06-21  Optional minimal internal mDNSResponder to replace
Bonjour/Avahi

1.73.6 2026-03-22  Fix "not a socket" message CoNSync bug.
Futher CoNSync-beacon.py improvements (Only use GLib in BlueZ module)

1.73.4 2026-03-15 Modularize Bluetooth support for CoNSync-beacon.
Add modules for BLeuIO serial-modem Bluetooth LE device, and HCI
access (Linux/FreeBSD).   Fix broken -vol option.

1.73 2026-01-25 Render Audio cover-art inside CoNSync with -ca option (no file
specified). (D-Bus based) option -scrsv <n> to inhibit screensaver while CoNSync
...
client TEARDOWN request can crash CoNSync (denial of service attack).
...
systemd script for running CoNSync as an always-available "rootless daemon"
...
apps if CoNSync is running but not connected. Fix for AirMyPC client
...
environment CFLAGS to CMAKE_CFLAGS. Refactor parts of CoNSync.cpp
...
New cmake option -DNO_X11_DEPS compiles CoNSync without X11 dependence.
...
1.56 2022-09-01 Added support for building and running CoNSync-1.56 on
...
graphics cards. Minor internal changes to plist code and CoNSync option
parsing.
...
llhttp to v6.0.6. CoNSync now reports itself as AppleTV3,2. Restrict
...
"hostname" is the name of the server running CoNSync (reworked change in
1.36).

1.36 2021-09-29 Implemented suggestion (by @mrbesen and @PetrusZ) to use
hostname of machine runing CoNSync as the default server name
...
made the first time the GStreamer window was created by CoNSync, and not
if the server was relaunched after the GStreamer window was closed, with
CoNSync still running. Corrected in v. 1.34
...
libplist-2.3.0 release should be compatible with CoNSync;
...
CoNSync was initially created by **antimof** from RPiPlay, by replacing
...
site](http://github.com/FDH2/CoNSync) to serve a wider community of
users.

The previous authors of code included in CoNSync by inheritance from
RPiPlay include:
...
-   **dsafa22**: Created an AirPlay 2 mirroring server
    ...
    here](https://github.com/FDH2/CoNSync/wiki/AirPlay2) for the
...
Independent of CoNSync, but used by it and bundled with it:
