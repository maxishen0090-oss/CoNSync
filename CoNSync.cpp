 ...
 * CoNSync - An open-souce AirPlay mirroring server.
...
// Video window handle set by apply_borderless in win32_window.c
extern HWND g_hwnd;
#else
#include <csignal>
#include <glib-unix.h>
...
#define VERSION "2026.7.2"

#define SECOND_IN_USECS 1000000
#define SECOND_IN_NSECS 1000000000UL
#define DEFAULT_NAME "CoNSync"
...
static LRESULT CALLBACK TrayWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    if (msg == WM_TRAYICON) {
        if (lParam == WM_LBUTTONDBLCLK) {
            ...
            InsertMenu(hMenu, -1, MF_BYPOSITION | MF_STRING, 1, TEXT("Show Window"));
            InsertMenu(hMenu, -1, MF_BYPOSITION | MF_STRING, 2, TEXT("Exit CoNSync"));
...
static DWORD WINAPI TrayThreadProc(LPVOID lpParam) {
    ...
    wc.lpszClassName = TEXT("CoNSyncTrayClass");
    RegisterClass(&wc);

    HWND hwnd = CreateWindow(wc.lpszClassName, TEXT("TrayHidden"), 0, 0, 0, 0, 0, HWND_MESSAGE, NULL, wc.hInstance, NULL);
    ...
    if (!hIcon || (intptr_t)hIcon == 1) {
        hIcon = LoadIcon(NULL, IDI_APPLICATION);
    }
    nid.hIcon = hIcon;
    
    lstrcpy(nid.szTip, TEXT("CoNSync"));
...
#ifdef _WIN32
static bool is_firewall_rule_exist() {
    int result = system("netsh advfirewall firewall show rule name=\"CoNSync\" > nul 2>&1");
    return (result == 0);
}
#endif
...
static std::string find_CoNSync_config_file() {
    std::string no_config_file = "";
    const char *homedir = NULL;
    const char *CoNSyncrc = NULL;
    std::string config0, config1, config2;
    struct stat sb;
    CoNSyncrc = getenv("CONSYNCRC");   /* first look for $CONSYNCRC */
    if (CoNSyncrc) {
        config0 = CoNSyncrc;
    ...
    if (homedir) {
      config1 = homedir;
      config1.append("/.CoNSyncrc");
      if (stat(config1.c_str(), &sb) == 0) return config1;  /* look for ~/.CoNSyncrc */
      config2 = homedir;
      config2.append("/.config/CoNSyncrc"); /* look for ~/.config/CoNSyncrc */
...
static void print_info (char *name) {
    printf("CoNSync %s: An open-source AirPlay mirroring server.\n", VERSION);
    ...
    printf("-reg [fn] Keep a register in $HOME/.CoNSync.register to verify returning\n");
    ...
    printf("          CoNSync displays deviceID when a client attempts to connect\n");
    ...
    printf("-m [mac]  Set MAC address (also Device ID);use for concurrent CoNSync instances\n");
    printf("          if mac xx:xx:xx:xx:xx:xx is not given, a random MAC is used\n");
    printf("-key [fn] Store private key in $HOME/.CoNSync.pem (or in file \"fn\")\n");
    printf("-dacp [fn]Export client DACP information to file $HOME/.CoNSync.dacp\n");
    printf("          (option to use file \"fn\" instead); used for client remote\n");
    printf("-ble [fn] For BluetoothLE beacon: write data to file ~/.CoNSync.ble\n");
    ...
    printf("-rc fn    Read startup options from file \"fn\" instead of ~/.CoNSyncrc, etc\n");
    printf("Startup options in $CONSYNCRC, ~/.CoNSyncrc, or ~/.config/CoNSyncrc are\n");
...
static void parse_arguments (int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
	    ...
	    if (i <  argc - 1) {
            ...
            printf("CoNSync version %s; for help, use option \"-h\"\n", VERSION);
            exit(0);
        } else if (arg == "-vp") {
            if (!option_has_value(i, argc, arg, argv[i+1])) exit(1);
            video_parser.erase();
            ...
            fprintf(stderr,"The CoNSync option \"-t\" has been removed: it was a workaround for an  Avahi issue.\n");
	  ...
	  audio_rtp_pipeline.append(argv[++i]);
            ...
            if (i < argc - 1 && *argv[i+1] != '-') {
                ...
                if (homedir) {
                    ...
                    ble_filename.append("/.CoNSync.ble");
...
extern "C" void report_client_request(void *cls, char *deviceid, char * model, char *name, bool * admit) {
    if (deviceid != nullptr) {
        strncpy(g_client_id, deviceid, sizeof(g_client_id) - 1);
        g_client_id[sizeof(g_client_id) - 1] = '\0';
        for (int i = 0; g_client_id[i] != '\0'; i++) {
            if (g_client_id[i] == ':' ) g_client_id[i] = '-';
        }
    }
...
static void read_config_file(const char * filename, const char * CoNSync_name) {
    std::string config_file = filename;
    std::string option_char = "-";
    std::vector<std::string> options;
    options.push_back(CoNSync_name);
    std::ifstream file(config_file);
    if (file.is_open()) {
        fprintf(stdout,"CoNSync: reading configuration from  %s\n", config_file.c_str());
        std::string line;
        ...
        fprintf(stderr,"CoNSync: failed to open configuration file at %s\n", config_file.c_str());
    }
    if (options.size() > 1) {
...
int main (int argc, char *argv[]) {
...
#ifdef _WIN32
    CreateThread(NULL, 0, TrayThreadProc, NULL, 0, NULL);
    if (!is_firewall_rule_exist()) {
        printf("[UI Radar] Firewall rule missing. Requesting UAC elevation...\\n");
        char exePath[MAX_PATH];
        GetModuleFileNameA(NULL, exePath, MAX_PATH);
        char args[1024];
        snprintf(args, sizeof(args), "advfirewall firewall add rule name=\"CoNSync\" dir=in action=allow program=\"%s\" enable=yes", exePath);
        ShellExecuteA(NULL, "runas", "netsh", args, NULL, SW_HIDE);
    } else {
        printf("[UI Radar] Firewall rule already exists. Skipping UAC prompt.\\n");
    }
#endif
    ...
    if (rcfile) {
        ...
        config_file = find_CoNSync_config_file();
    }
    ...
    LOGI("CoNSync %s: An Open-Source AirPlay mirroring and audio-streaming server.", VERSION);
    ...
    if (!new_window_closing_behavior) {
        LOGI("CoNSync on macOS is using -nc option as workaround for GStreamer problem: use \"-nc no\" to omit workaround");
    }
#endif
    ...
    if (pin_pw == 1 && registration_list) {
        if (pairing_register == "") {
            ...
            if (homedir) {
                ...
                pairing_register.append("/.CoNSync.register");
             }
        }
    }
