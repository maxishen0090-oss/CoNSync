...
-   \*\*NEW in v 1.74 (Experimental) (June 2026)
    ...
    **This is a trial of a future release with new contributed code that
    ...
    available by compiling CoNSync using `cmake -DUSE_DNS_SD=1`** (By
    default, CoNSync on macOS continues to use Bonjour: to test the
    CoNSync internal mdns implementation on macOS, use
    `cmake -DUSE_MDNS=1`).
    ...
    *Comments about issues with this new internal mdns implementation,
    ...
    [#529](https://github.com/FDH2/CoNSync/issues/529)
...
-   Support for **service discovery using a Bluetooth LE "beacon"** for
    ...
    script `CoNSync-beacon.py`. Loadable Python modules provide
    ...
    *macOS is only supported by the BleuIO USB dongle which uniquely has
    ...
    systems supported by CoNSync, including other BSD variants).* The
    beacon runs independently of CoNSync. While CoNSync is running, the
    ...
    local IPv4 network address of the CoNSync server, and which TCP port
    to contact CoNSync on. Instructions are [given
    below](#bluetooth-le-beacon-setup).

-   option `-vrtp <rest-of-pipeline>` bypasses rendering by CoNSync, and
...
-   (for Linux/\*BSD Desktop Environments using D-Bus). New option
    ...
    (always on while CoNSync is running). Tested on
...
-   option -ca (with no filename given) will now render Apple Music
    cover art (in audio-only mode) inside CoNSync. (-ca `<filename>` will
...
-   Improved Support for (YouTube) HLS (HTTP Live Streaming) video with
    ...
    video. **Please report any issues with this new feature of CoNSync**.
...
-   If your distribution does not supply CoNSync, or you want the latest
    version, it is very easy to build it yourself: see the very
    [detailed instructions for building CoNSync from
    source](#building-CoNSync-from-source). later in this document.
...
-   For Audio-only mode (Apple Music, etc.) best quality is obtained
    ...
    latency imposed by iOS. Use option "CoNSync -ca" to display any
...
-   If you are using CoNSync just to mirror the client's screen (without
    ...
    to use the option "CoNSync -vsync no".
...
-   Add any CoNSync options you want to use as defaults to a startup file
    ...
    other possible locations; the location can also be set with "CoNSync
...
-   For Linux/\*BSD systems using D-Bus, the option `-scrsv 1` inhibits
    the screensaver while there is video activity on CoNSync (`-scrsv 2`
    inhibits it whenever CoNSync is running).

-   For Linux systems using systemd, there is a **systemd** service file
    **CoNSync.service** found in the CoNSync top directory of the
    distribution, and also installed in `<DOCDIR>/CoNSync/systemd/`
    (where DOCDIR is usually `/usr/local/share/doc`), that allows users
    to start their own instance of CoNSync as a rootless daemon: it
    should either be added to the directory /etc/systemd/user, or the
    user can just create their own systemd directory
    `~/.config/systemd/user/` and then copy CoNSync.service into it. To
    save CoNSync terminal output to a file \~/CoNSync.log, uncomment the
    StandardOutput entry in CoNSync.service. Then
    ...
    simultaneously run CoNSync this way, they should specify distinct -p
    and -m options (ports and deviceID) in their startup files. **Note:
    it is NOT recommended to run CoNSync as a root service.**

-   On Raspberry Pi: models using hardware h264 video decoding by the
    Broadcom GPU (models 4B and earlier) may require the CoNSync option
    -bt709. If you use Ubuntu 22.10 or earlier, GStreamer must be
    [patched](https://github.com/FDH2/CoNSync/wiki/Gstreamer-Video4Linux2-plugin-patches)
...
-   If CoNSync is used in a public space, there are security options for
...
"`rpmdev-setuptree`". Then download and copy CoNSync.spec into
`~/rpmbuild/SPECS`. In that directory, run
"`pmdev-spectool -g -R  CoNSync.spec`" to download the corresponding
...
-   A method to enable Service Discovery (so AirPlay clients on the
    local network can find the AirPlay service provided by CoNSync) must
    be set up. The standard method used by CoNSync is to announce the
    ...
    Previously, CoNSync needed an external `DNS_SD` service (Apple's
    Bonjour service, usually implemented on Linux/\*BSD by Avahi or
    mDNSResponder, but CoNSync now also supplies its own built-in
...
If the CoNSync server is not seen by the iOS client's drop-down "Screen
...
If CoNSync is seen, but the client fails to connect when it is selected,
...
-   Unlike an Apple TV, the CoNSync server does not by default require
    ...
    enter it on their iOS/macOS device to access CoNSync, when prompted
...
-   You can now display (inside CoNSync) the accompanying "Cover Art"
    from sources like Apple Music in Audio-Only (ALAC) mode with the
    option `CoNSync -ca`. *The older method of exporting cover art to an
    external viewer remains available: run "`CoNSync -ca <name> &`" in
    the background, then run a image viewer with an autoreload feature:
    an example is "feh": run "`feh -R 1 <name>`" in the foreground;
    terminate feh and then CoNSync with "`ctrl-C fg ctrl-C`"*.
...
latest macOS, CoNSync can run on older macOS systems that will cannot run
...
longer be relevant: *For X11 support on macOS, compile CoNSync using a
...
when using X11: `CoNSync -s 800x600`.)*
...
-   **NEW**: CoNSync now optionally supplies its own minimal
    ...
    service on macOS. To use the CoNSync implementation (which is tested
...
-   Running CoNSync while checking for GStreamer warnings (do this with
    ...
    GStreamer warnings. **Recommendation: use the CoNSync "no timestamp"
    option "`-vsync no`"** (you can add a line "vsync no" in the
    CoNSyncrc configuration file).
...
-   *On older releases of GStreamer, there were issues involving windows
    not closing properly, that required the CoNSync option "-nc": these
...
-   **NEW: CoNSync now supplies its own self-contained mdns replacement
...
Now test by running "`CoNSync`" (in a MSYS2 UCRT64 terminal window. If
...
-   On Windows, with Direct3D 11.0 or greater, various options can be
    ...
    their CoNSync startup file, to get this toggle option (autovideosink
...
this overrides `$CONSYNCRC`, `~/.CoNSyncrc`, etc.
...
YouTube videos can be streamed directly from YouTube servers to CoNSync
...
should list CoNSync when it is inhibiting the screensaver.*
...
at least six characters) is set when CoNSync starts (usually set in the
startup file, where it is stored as cleartext), all users must know this
password to connect to CoNSync (the client prompts for it). This method
uses HTTP md5 Digest authentication, which is now regarded as providing
weak security, but it is only used to validate the CoNSync password, and
...
authentications without prompting, so long as the CoNSync deviceID has
not changed (this initial authentication will fail if the CoNSync
password has changed). If *pwd* is **not** specified with the -pw option
when CoNSync starts, a new random 4-digit pin code is generated and
displayed on the CoNSync terminal for **each** new connection, and must
...
metadata is received, and the file is deleted when CoNSync terminates.
...
port, PID and process name of the CoNSync process is recorded by default
in `~/.CoNSync.ble` : (this file is created when CoNSync starts and
...
GST_DEBUG=2" before running CoNSync. To see GStreamer information
...
beacon is CoNSync-beacon.py. Besides the standard python libraries, it
requires python3-psutils.

It provides four possible Bluetooth LE implementations (loaded as
modules `CoNSync_beacon_module_*.py`), which may need additional
libraries:
...
-   **HCI** For Linux and FreeBSD: no extra libraries, but requires root
    ...
    and you can find it on the [CoNSync
    Wiki](https://github.com/FDH2/CoNSync/wiki/hccontrol-patch-for-FreeBSD-15.0).
...
CoNSync-beacon.py. On Debian-based systems:
...
CoNSync must be run with option "`CoNSync -ble [path_to_file]`" (so it
writes data for the Bluetooth beacon to the default BLE data file
`~/.CoNSync.ble`, or optionally to the file specifed by its path), then
run `CoNSync-beacon.py [HCI, BleuIO] [--path path_to_file]` in a separate
...
when it detects that CoNSync is running by checking if the BLE data file
exists, and stop when it no longer detects a running CoNSync (it will
restart advertising if CoNSync later reappears). The script will remain
...
file `~/.CoNSync.beacon`, if it exists. Configuration file entries are
...
Get help with `man CoNSync-beacon` or `CoNSync-beacon.py --help`.
...
-   `--file <config file>` read beacon options from `<config file>`
    instead of `~/.CoNSync.beacon`.

-   `--ipv4  <ipv4 address>`. This option can be used to specify the
    ipv4 address at which the CoNSync server should be contacted by the
...
-   `--path <BLE data file>`. This overrides the default choice of BLE
    data file (`~/.CoNSync.ble`) that is monitored by the beacon script.
    This also requires that CoNSync is run with option
    "`CoNSync -ble <BLE data file>`".
...
-   `--index x` (default x = 0, x \>= 0). This can be used by the DBus
    ...
    CoNSync-beacon.py that are running to support multiple instances of
    CoNSync. Each instance must have its own BLE Data file (just as each
    instance of CoNSync must also have its own MAC address and ports).
...
CoNSync-beacon.py on macOS uses a BleuIO USB serial device.
...
see the [wiki
page](https://github.com/FDH2/CoNSync/wiki/Bluetooth_LE_beacon).
...
`export GST_VAAPI_ALL_DRIVERS=1` before running CoNSync, or set this in
...
If CoNSync issues no error messages, but GStreamer video seems not to be
...
`man CoNSync` for a list) to see which work on your system. Also use
...
1.73.6 2026-03-22 Fix "not a socket" message CoNSync bug. Futher
CoNSync-beacon.py improvements (Only use GLib in BlueZ module)

1.73.4 2026-03-15 Modularize Bluetooth support for CoNSync-beacon. Add
modules for BLeuIO serial-modem Bluetooth LE device, and HCI access
(Linux/FreeBSD). Fix broken -vol option.

1.73 2026-01-25 Render Audio cover-art inside CoNSync with -ca option (no
file specified). (D-Bus based) option -scrsv `<n>`{=html} to inhibit
screensaver while CoNSync is running (Linux/\*BSD only). Add password
...
specially-crafted client TEARDOWN request can crash CoNSync (denial of
service attack).
...
user-installable systemd script for running CoNSync as an
