 ...
 * Modified for:
 * CoNSync - An open-source AirPlay mirroring server
...
#ifdef _WIN32
    win32_set_aspect_ratio((int)*f_width_source, (int)*f_height_source);
#endif
...
void video_renderer_init(logger_t *render_logger, const char *server_name, videoflip_t videoflip[2], const char *parser, const char * rtp_pipeline,
    ...
    /* (instead of the program name CoNSync taken from (argv[0]). It is only set one time. */
    ...
    for (int i = 0; i < n_renderers; i++) {
        ...
        if (hls_video) {
            ...
            if (error) {
                ...
                if (strstr(error->message, "no element")) {
                    logger_log(logger, LOGGER_ERR, "This error usually means that a CoNSync option was mistyped\n"
         ...
         * (a CoNSync option to specify a different value is possible)  */
...
uint64_t video_renderer_render_buffer(unsigned char* data, int *data_len, int *nal_count, uint64_t *ntp_time) {
    ...
    if (!renderer || !(renderer->appsrc)) {
        ...
        if (renderer->gst_window && !(renderer->gst_window->window) && renderer->use_x11) {
            X11_search_attempts++;
            logger_log(logger, LOGGER_DEBUG, "Looking for X11 CoNSync Window, attempt %d", (int) X11_search_attempts);
            get_x_window(renderer->gst_window, renderer->server_name);
            if (renderer->gst_window->window) {
    ...
    case GST_MESSAGE_ERROR: {
        ...
        if (!hls_video && strstr(err->message,"Internal data stream error")) {
            logger_log(logger, LOGGER_INFO,
                     ...
                     "*** to select a videosink of your choice (see \"man CoNSync\").\n\n"
                     "*** Raspberry Pi models 4B and earlier using Video4Linux2 may need \"-bt709\" CoNSync option");
        }
        g_error_free (err);
        g_free (debug);
        if (renderer->appsrc) {
