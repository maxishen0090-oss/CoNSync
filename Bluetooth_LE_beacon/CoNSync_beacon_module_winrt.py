...
#---------------------------------------------------------------
# winrt (Windows) module for a standalone python-3.6 AirPlay Service-Discovery Bluetooth LE beacon for CoNSync
...
def setup_beacon(ipv4_str: str, port:int , advmin: Literal[None], advmax :Literal[None], index :Literal[None]) ->bool:
    ...
    if (advmin is not None) or (advmax is not None) or (index is not None):
        raise ValueError('CoNSync_beacon_module_winrt: advmin, advmax, index were not all None')
...
print(f'loaded CoNSync_beacon_module_winrt')
