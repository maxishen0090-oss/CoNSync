...
#----------------------------------------------------------------
# a standalone python-3.6 or later AirPlay Service-Discovery Bluetooth LE beacon for CoNSync 
# (c)  F. Duncanh, October 2025

import sys
if not sys.version_info >= (3,6):
    print("CoNSync-beacon.py requires Python 3.6 or higher")
        ...
        if test:
            if not beacon_is_running:
                ...
                if not check_port(port):
                    # CoNSync is active, and beacon is running but is advertising a different port, so shut it down
...
def parse_params():
    ...
    default_config_file = home_dir+"/.CoNSync.beacon"
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='beacon startup file (Default: ~/.CoNSync.beacon).'
    )
    
    parser.add_argument(
        '--path',
        type=str,
        default= home_dir + "/.CoNSync.ble", 
        help='path to AirPlay server BLE beacon information file (default: ~/.CoNSync.ble).'
    )
    parser.add_argument(
        '--ipv4',
        type=str,
    ...
    if config_file is not None:
        print("Read CoNSync-beacon.py configuration file ", config_file)
        try:
...
if __name__ == '__main__':
    ...
    module = f'CoNSync_beacon_module_{ble_type}'
