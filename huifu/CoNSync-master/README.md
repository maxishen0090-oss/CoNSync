...
### **Now developed at the GitHub site <__CONSYNC_URL_0__> (where ALL user issues should be posted, and latest versions can be found).**
...
-   **NEW in v 1.74 (Experimental) (June 2026)
     ...
     *Comments about issues with this new internal mdns implementation, and
     ...
     are extremely welcome.*  (Issue [#529](__CONSYNC_URL_1__)
...
-   For other RPM-based distributions which have not yet packaged
    ...
    [releases](__CONSYNC_URL_2__) (see their
...
pulled from the new main [CoNSync site](__CONSYNC_URL_4__)).
...
[CoNSync-master.zip](__CONSYNC_URL_7__),
or (if git is installed): "git clone __CONSYNC_URL_8__".
You can also download a recent or earlier version listed in
[Releases](__CONSYNC_URL_9__).

-   A recent CoNSync can also be found on the original [antimof
    site](__CONSYNC_URL_10__); that original project is
    inactive, but is usually kept current or almost-current with the
    [active CoNSync github site](__CONSYNC_URL_11__) (thank
    you antimof!).
...
-   Most users will use the GStreamer supplied by their distribution,
    ...
    instructions](__CONSYNC_URL_12__).)
...
-   For "double-legacy" Raspberry Pi OS (Buster), there is no patch for
    ...
    instructions](__CONSYNC_URL_14__).)
...
4.  Download the latest CoNSync from github **(to use `git`, install it
    ...
    "`git clone __CONSYNC_URL_15__")**, then install CoNSync
...
* **HCI** For Linux and FreeBSD: no extra libraries, but requires root access to the HCI (Host Controller Interface) for Bluetooth devices, using utilities
    ...
    and you can find it on the [CoNSync Wiki](__CONSYNC_URL_16__). _Note that macOS
...
For more information on Bluetooth LE support, including HCI commands, see the [wiki page](__CONSYNC_URL_17__).
...
site](__CONSYNC_URL_20__) to serve a wider community of
users.
...
-   **dsafa22**: Created an AirPlay 2 mirroring server
    ...
    here](__CONSYNC_URL_21__) for the
